...
async function ensureTestWorkspace() {
  const existing = await prisma.workspace.findUnique({ where: { ownerId: E2E_USER_ID } })
  if (existing) return existing
  return prisma.workspace.create({
    data: {
      ownerId: E2E_USER_ID,
      name: "E2E Workspace",
      slug: `e2e-ws-${Math.random().toString(36).slice(2, 8)}`,
      members: { create: { userId: E2E_USER_ID, role: "owner" } },
    },
  })
}

async function getE2eWorkspaceId(): Promise<string> {
  const ws = await prisma.workspace.findUnique({
    where: { ownerId: E2E_USER_ID },
    select: { id: true },
  })
  if (!ws) throw new Error("E2E workspace missing — ensureTestUser wasn't called")
  return ws.id
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const e2eWorkspaceId = await getE2eWorkspaceId()
