...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true)
      try {
        const res = await fetch(`/api/workspace/search?q=${encodeURIComponent(searchQuery)}`)
        if (res.ok) {
          const data = await res.json()
  ...
  const deleteWorkspace = async () => {
    if (!confirm("Are you sure you want to delete your workspace? All members will be removed.")) {
      return
    }
    setActionLoading(true)
    setError(null)
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        {!workspace && (
          <div className="rounded-lg border border-border bg-card p-8 text-center">
            ...
            <h2 className="text-lg font-medium text-foreground mb-2">Create a Workspace</h2>
            <p className="text-sm text-muted-foreground mb-6 max-w-sm mx-auto">
              Create a workspace to share your Claude subscription with others.
              Workspace members will automatically use your Claude credentials.
            </p>
            <button
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-medium text-foreground">Add Workspace Member</h2>
              </div>
            ...
            <div className="rounded-lg border border-border bg-card">
              ...
              {workspace.members && workspace.members.length > 0 ? (
                ...
                <div className="px-4 py-8 text-center text-sm text-muted-foreground">
                  No members yet. Add workspace members to share your Claude subscription.
                </div>
              )}
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    This will remove all members from your workspace.
                  </p>
                </div>
                <button
        ...
        {workspace && !workspace.isOwner && workspace.owner && (
          <div className="space-y-6">
            <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-6 text-center">
              <div className="flex justify-center mb-4">
                {workspace.owner.image ? (
                  <img src={workspace.owner.image} alt="" className="h-16 w-16 rounded-full" />
                ) : (
                  <div className="h-16 w-16 rounded-full bg-secondary" />
                )}
              </div>
              <h2 className="text-lg font-medium text-foreground mb-1">
                Using {workspace.owner.name || workspace.owner.githubLogin}&apos;s Claude Subscription
              </h2>
              <p className="text-sm text-muted-foreground mb-6">
                You&apos;re a member of @{workspace.owner.githubLogin}&apos;s workspace.
