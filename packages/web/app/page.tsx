...
export default function Home() {
  ...
  // First-run redirect: if the signed-in user has zero repos configured,
  // bounce them to the onboarding page. Guarded by a sessionStorage flag
  // so it only fires once per tab (avoids loops and lets them back-nav
  // freely afterward).
  useEffect(() => {
    if (status !== "authenticated" || !loaded) return
    if (repos.length > 0) return
    if (typeof window === "undefined") return
    const flag = "charclaw.setupNudgeShown"
    if (window.sessionStorage.getItem(flag)) return
    window.sessionStorage.setItem(flag, "1")
    router.replace("/setup")
  }, [status, loaded, repos.length, router])
  ...
  useEffect(() => {
    ...
    if (repoPrefix) {
      if (totalNotifications > 0) {
        document.title = `${repoPrefix} (${totalNotifications}) – CharClaw`
      } else {
        document.title = `${repoPrefix} – CharClaw`
      }
    } else {
      document.title = "CharClaw"
    }
  }, [repos, activeRepo])
