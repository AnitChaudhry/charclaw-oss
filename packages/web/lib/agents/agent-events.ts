...
import { scheduleSkillExtraction } from "@/lib/skills/extract-skill"
...
export async function persistExecutionCompletion(
  ...
  // Fire skill extraction asynchronously after a successful completion
  if (result.status === "completed" && content && execution.message?.branchId) {
    const branch = await prisma.branch.findUnique({
      where: { id: execution.message.branchId },
      select: { name: true, repo: { select: { userId: true, name: true } } },
    })
    if (branch) {
