...
async function main() {
  ...
  try {
    ...
    await bg.start("My name is Anit.")
    ...
    if (combinedText.includes("anit")) {
      console.log("✅ Background memory test passed (found 'anit').")
      process.exit(0)
    } else {
      console.error("❌ Background memory test FAILED (did not find 'anit').")
      process.exit(1)
    }
  } finally {
