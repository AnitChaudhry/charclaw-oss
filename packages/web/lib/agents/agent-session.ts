 ...
 * - System prompt building (via @charclaw/common)
 * - Tool name mapping (via @charclaw/common)
 * - Event transformation (via @charclaw/common)
 * - Content blocks reconstruction (via @charclaw/common)
 * - Session persistence
 ...
 * This is a thin wrapper around @charclaw/common's buildContentBlocks
...
export async function readPersistedSessionId(
  sandbox: DaytonaSandbox | { executeCommand?: (cmd: string, timeout?: number) => Promise<{ exitCode: number; output: string }> }
): Promise<string | undefined> {
  try {
    // CodeAgentSandbox (local or Daytona-adapted) uses executeCommand
    if ("executeCommand" in sandbox && typeof sandbox.executeCommand === "function") {
      const result = await sandbox.executeCommand(`cat ~/.agent_session_id 2>/dev/null`, 10)
      if (result.exitCode === 0 && result.output.trim()) {
        return result.output.trim()
      }
      return undefined
    }
    // Legacy: raw Daytona sandbox with .process
    const daySandbox = sandbox as DaytonaSandbox
...
export async function createBackgroundAgentSession(
  // Accepts either the raw Daytona SDK Sandbox (legacy callers) or the
  // abstracted wrapper. The helpers below tolerate either — the body
  // already casts to `any` when passing through to the SDK.
  sandbox: DaytonaSandbox | CodeAgentSandbox,
  ...
  // Map agent type to SDK provider name. No hardcoded default — the caller
  // must have resolved agent from user selection / branch state before
  // getting here. If options.agent is unset it indicates a bug upstream.
  if (!options.agent) {
    throw new Error(
      "createBackgroundAgentSession: options.agent is required (no silent fallback)",
    )
  }
