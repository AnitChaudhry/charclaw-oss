...
/**
 * Eliza is a fake in-process agent — it has no CLI to install and is always
 * "available" regardless of what the daemon reports.
 */
const ALWAYS_AVAILABLE: ReadonlySet<Agent> = new Set(["eliza"])

/**
 * Returns true when the agent's CLI is installed on at least one runtime —
 * or when `installedAgents` is null/undefined (caller has no runtime data,
 * fall back to "show all" behavior).
 */
export function isAgentInstalled(
  agent: Agent,
  installedAgents: ReadonlySet<Agent> | Agent[] | null | undefined,
): boolean {
  if (installedAgents == null) return true
  if (ALWAYS_AVAILABLE.has(agent)) return true
  if (Array.isArray(installedAgents)) return installedAgents.includes(agent)
  return installedAgents.has(agent)
}

export type AgentDisabledReason =
  | { kind: "needs-cli"; agent: Agent; tooltip: string }
  | { kind: "needs-credentials"; agent: Agent; tooltip: string }
  | null

/**
 * Returns null when the agent is fully usable. Otherwise returns the most
 * blocking reason — CLI install missing wins over credentials missing,
 * because credentials are useless without the CLI.
 */
export function getAgentDisabledReason(
  agent: Agent,
  installedAgents: ReadonlySet<Agent> | Agent[] | null | undefined,
  credentials: UserCredentialFlags | null | undefined,
): AgentDisabledReason {
  if (!isAgentInstalled(agent, installedAgents)) {
    return {
      kind: "needs-cli",
      agent,
      tooltip: `${agentLabels[agent]} CLI not installed on your runtime — open Setup to install it.`,
    }
  }
  const hasCreds = hasCredentialsForAgent(agent, credentials)
  if (!hasCreds) {
    return {
      kind: "needs-credentials",
      agent,
      tooltip: `${agentLabels[agent]} needs an API key — add it in Settings.`,
    }
  }
  return null
}

/** Checks if the user has any credentials usable by the given agent. */
export function hasCredentialsForAgent(
  agent: Agent,
  credentials: UserCredentialFlags | null | undefined,
): boolean {
  switch (agent) {
    case "claude-code":
      return hasClaudeCodeCredentials(credentials)
    case "codex":
      return hasCodexCredentials(credentials)
    case "gemini":
      return hasGeminiCredentials(credentials)
    case "goose":
      return hasGooseCredentials(credentials)
    case "pi":
      return hasPiCredentials(credentials)
    case "eliza":
      return hasElizaCredentials(credentials)
    case "opencode":
      // OpenCode has free models — always considered "credential-ok" at the
      // agent level. Per-model credential gating is enforced by
      // hasCredentialsForModel when the user picks a specific paid model.
      return true
  }
}
