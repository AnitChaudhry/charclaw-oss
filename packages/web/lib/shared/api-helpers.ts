...
/**
 * Resolves and decrypts user credentials, checking workspace membership.
 * The user's OWN Anthropic credentials always take priority — workspace
 * owner's Claude subscription is only used as a fallback when the user
 * has neither an `anthropicApiKey` nor an `anthropicAuthToken` of their own.
 *
 * @param credentials - The user's credentials from database
 * @param userId - The user's ID (needed to check workspace membership)
 * @returns Decrypted credentials (own when present, else owner's anthropicAuthToken)
 */
export async function resolveUserCredentials(
  ...
  // User's own Anthropic creds win — no need to consult the workspace owner.
  const hasOwnAnthropic = Boolean(
    ownCredentials.anthropicApiKey || ownCredentials.anthropicAuthToken
  )
  if (hasOwnAnthropic || !userId) {
    return ownCredentials
  }

  // Fallback only: user is a member of someone else's workspace and has no
  // Anthropic creds of their own — inherit the owner's Claude subscription.
  const membership = await prisma.workspaceMember.findFirst({
    where: {
      userId,
      workspace: { NOT: { ownerId: userId } },
    },
    include: {
      workspace: {
        include: {
  ...
  if (membership?.workspace?.owner?.credentials?.anthropicAuthToken) {
    return {
      ...ownCredentials,
      anthropicAuthToken: decrypt(membership.workspace.owner.credentials.anthropicAuthToken),
