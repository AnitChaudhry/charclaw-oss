...
import { agentLabels, getModelLabel, defaultAgentModel, getAvailableModels, hasClaudeCodeCredentials, hasCodexCredentials, hasGeminiCredentials, hasPiCredentials, hasCredentialsForModel, agentModels, getAgentDisabledReason, isAgentInstalled } from "@/lib/shared/types"
...
import { filterSlashCommands } from "@charclaw/common"
...
export const ChatInput = forwardRef<HTMLTextAreaElement, ChatInputProps>(
  function ChatInput(
    ...
    // Which agents have a CLI installed on the runtime that will actually
    // run this branch's turn? Scoped to branch.sandboxId when present so
    // a multi-runtime user doesn't see false positives from other runtimes.
    // Falls back to the union across all runtimes when no sandbox exists yet.
    const { installedAgents } = useInstalledAgents({ sandboxId: branch.sandboxId })
    const currentAgentCliInstalled = isAgentInstalled(currentAgent, installedAgents)
    ...
    // Handle agent change - CLI-missing opens Settings runtimes pane, credential-missing
    // opens the relevant API-key field, otherwise switches normally.
    const handleAgentChange = useCallback((newAgent: Agent) => {
      // CLI not installed on any runtime → don't switch, just open Settings.
      // The user has to install the CLI on their runtime first.
      if (!isAgentInstalled(newAgent, installedAgents)) {
        onOpenSettings?.()
        return
      }

      // CLI is installed — proceed with the switch, optionally opening
      // Settings to highlight a missing credential.
    ...
    }, [onAgentChange, onOpenSettings, onOpenSettingsWithHighlight, canUseClaudeCode, canUseCodex, canUseGemini, canUsePi, installedAgents])
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" sideOffset={4} className="min-w-[160px] rounded-lg border border-border/60 py-0.5 shadow-md">
                {(Object.keys(agentLabels) as Agent[]).map((agent) => {
                  const reason = getAgentDisabledReason(agent, installedAgents, credentials)
                  ...
                  const item = (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        "flex items-center justify-between py-1.5 text-[11px] cursor-pointer",
                        dimmed && "opacity-50",
                      )}
                    >
                  ...
                  <CommandList>
                    {!currentAgentCliInstalled ? (
                      <button
                        ...
                        className="block w-full px-3 py-3 text-[11px] text-center text-muted-foreground hover:text-foreground cursor-pointer"
                      >
                        Install {agentLabels[currentAgent]} CLI on your runtime to use models.
                      </button>
                    ) : (
                      <>
